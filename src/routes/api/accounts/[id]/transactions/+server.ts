...
import { listFinancialAccountTransactions } from '$lib/server/financial-records';
...
export const GET: RequestHandler = async ({ params, url }) => {
	try {
		return apiJson(
			await listFinancialAccountTransactions(parseId(params.id), transactionLimit(url))
		);
	} catch (error) {

...
Tenant isolation is enforced in the application by a random tenant ID bound into each session, Plaid Item reference, and encrypted record payload. The shared Neon schema does not use row-level security, so every new data path must preserve these checks. Do not expose a second hostname casually, disable the request guards, or rely on Vercel Deployment Protection as the production login.
...
Infrastructure credentials are server-only and Production-only Vercel environment variables. Mark the database URL, AES key, password hash or temporary Google bootstrap verifier, Google client secret, and optional installation-level Plaid credentials as Sensitive. User-supplied Plaid client IDs and Production secrets are accepted only in authenticated, same-origin JSON requests, verified with Plaid, and stored as server-encrypted database values. Preview and Development environments must receive no production database, key, password hash, bootstrap verifier, Google secret, migration URL, recovery bundle, or real Plaid credential.

Google secrets, saved Plaid secrets, and long-lived provider tokens must never be returned to the browser. A user enters a Plaid Production secret in a password-type field and the Function necessarily processes that encrypted HTTPS body before encrypting it at rest; ChipDue does not log or echo it. The Google-only setup token follows the same handling constraints. Never place any credential in a variable beginning with `PUBLIC_` or `VITE_`, in a URL, or directly in a shell command.
...
- Enable phishing-resistant MFA where available for GitHub, Vercel, Neon, Google, each user's Plaid account, and the password manager holding the recovery bundle.

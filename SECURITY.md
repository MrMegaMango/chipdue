...
Google and Plaid secrets and long-lived tokens must never reach the browser. The Google-only setup token is manually pasted into its dedicated password-type field and sent once in a same-origin JSON body. The Vercel Function necessarily processes that encrypted HTTPS body, but CardDue must never put the token in a URL, cookie, browser storage, log, Google or Plaid request, or repository. Never place any credential in a variable beginning with `PUBLIC_` or `VITE_`, in a URL, or directly in a shell command. Use generated recovery and bootstrap bundles only from private encrypted storage outside every Git checkout.

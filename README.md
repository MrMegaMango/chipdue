...
- Compare each available synced purchase with a centered schedule of the same security and amount, investing half before its date and half after. The default uses three biweekly buys on each side (12 weeks); choose two, three, or six buys per side and monthly, weekly, or biweekly intervals. Recent purchases await their complete window. The hypothetical buy-and-hold model includes idle cash, uses adjusted daily closes, exposes each purchase's schedule and exclusions, and assumes funds were available before the recorded purchase. It does not represent realized trading profit.
...
Browser memory -> authenticated Vercel Function -> encrypted Neon Postgres rows
                                      ...
                                      +-> Yahoo Finance, for estimated history and investment comparisons
...
Use **Pause sync** beside an institution in Settings to exclude that connection from scheduled sync and bulk sync. Its saved balances, history, and last-sync time remain visible, and **Retry sync** can still refresh it individually. The encrypted pause setting belongs only to that connection in your ChipDue account and stays paused after a successful retry or repair until you choose **Resume sync**. Connections awaiting reconnection are also skipped by bulk sync; a failed connection does not hide successful updates from other institutions.

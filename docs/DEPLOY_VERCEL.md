...
The committed Vercel schedule refreshes every Plaid connection, grouped under its owning account and Plaid credentials, during the 8 AM and 5 PM hours in `America/Los_Angeles`. Four once-daily UTC candidates keep those two Pacific windows correct across daylight-saving changes. A candidate before its Pacific period exits without contacting Plaid; a later candidate can retry a missed or failed period. The dashboard's authenticated **Run** action can likewise catch up a period after its target hour. Each period is claimed atomically in `carddue_metadata`, so a later candidate or manual run does not repeat a completed sync. Vercel invokes each job on the generated production-deployment hostname rather than a stable project alias. That hostname bypasses the browser host allowlist only after the request has passed the same HTTPS requirement and Vercel Cron authentication used by the route itself. Vercel supplies `Authorization: Bearer CRON_SECRET`; the endpoint also requires Vercel's Cron user agent and otherwise remains unavailable. On Vercel Hobby, an invocation can occur anywhere within the scheduled hour; Pro and Enterprise schedules have per-minute precision.

...
Offline recovery bundle -> login password, password hash, AES key, and runtime DB password
```
...
The hosted model has one recovery password, at most one pre-linked Google identity, one production hostname, one Vercel project, and one Neon production database. It has no per-user accounts, sharing, delegated access, or public API clients. Preview builds are nonfunctional for real data because they receive no cloud, Google, or Plaid secrets.
...
- The cloud AES key, owner login password and hash, session and OAuth transaction cookies, and recovery bundle
...
- Public-internet password guessing, session theft, host-header abuse, and cross-origin requests
...
| Production secrets reach a preview       | Every secret is scoped to Vercel Production; previews receive no real database, AES key, password hash, Google secret, or Plaid credentials                                |
...
| Public requests bypass authentication    | Every private cloud API is authorized server-side; sessions are random, stored only as hashes, password-bound, expiring, revocable, Secure, HTTP-only, and SameSite Strict |
| Password guessing                        | Memory-hard password hash, generic failures, persistent keyed rate-limit buckets, and bounded lockout window                                                               |
| Google flow claims or links an owner     | Google can be linked only from an existing exact CardDue session; the first visitor can never provision or replace the owner                                               |
...
- A compromised linked Google account authenticating to CardDue; the independent recovery password remains available if Google is unavailable
...
- No in-application Google unlink or rebind flow; incident containment requires disabling both Google variables and redeploying before password/session recovery

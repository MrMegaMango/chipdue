...
ChipDue is built around data minimization. Manual entry is fully functional without Plaid or E*TRADE, and local mode remains available without a hosted database.
...
- For Plaid connections, encrypted access tokens, Item IDs, selected account details, current and historical brokerage balances, automatically calculated period contributions or optional user-entered lifetime contributions, and optional investment holding cost basis
...
It does not store full account numbers, credentials entered into Plaid Link, identity profiles, addresses, locations, original bank descriptions, merchant logos, counterparties, raw Plaid responses, or raw E*TRADE responses. Plaid responses are mapped immediately to the allowlisted account, card, holding, and transaction fields, encrypted as application records, and then discarded. E*TRADE account and order responses are reduced to allowlisted display fields and kept only in browser memory for the current page. Portfolio and transaction inputs used for historical reconstruction are discarded after processing; the resulting estimated daily values, automatically calculated contribution values, and estimated provenance are stored within the encrypted account payload.
...
- For a connected brokerage with investment activity, ChipDue can combine those encrypted Plaid holdings and transactions with historical market closes to reconstruct up to two years of clearly labeled estimated values. The resulting estimates are stored inside the encrypted account payload; they are not balances reported by Plaid or the institution.
...
E*TRADE is optional in both modes. When configured, ChipDue sends signed server-to-server requests to E*TRADE to obtain a five-minute request token, exchange the verifier you paste after approving on E*TRADE's site, list brokerage accounts, fetch orders whose status is open, and—only when historical estimates are built—fetch current portfolio positions and up to two years of transactions. ChipDue encrypts the consumer credentials and tokens; it does not store the returned account list, orders, portfolio response, or transaction response. Access tokens expire at midnight Eastern and can require renewal after inactivity. The integration is read-only and has no operation for placing, changing, previewing, or cancelling a trade.

To value reconstruction dates, ChipDue makes server-to-server requests to Yahoo Finance's public chart endpoint. Each request contains only one ticker, a date range, and a daily interval. Yahoo does not receive the ChipDue, Plaid, or E*TRADE account identifier, holdings quantity, transaction details, or balance. This price source is best-effort and unofficial; failures or symbol mismatches leave the estimate incomplete rather than silently treating missing prices as authoritative.

The investment comparison on Accounts automatically requests SPY dividend-adjusted closing prices for the recorded history window. These requests contain only the fixed SPY ticker, dates, and daily-price options. A bounded server memory cache retains public prices for up to 15 minutes; it contains no account records. Account comparisons are calculated in browser memory, including estimated cash-flow timing, and unavailable prices or contribution history are shown as gaps rather than assumed returns.

The buy-timing comparison uses saved investment activity within the signed-in account. Its server endpoint accepts an owned account identifier, a preset purchase period, a buying frequency, and a bounded number of installments on each side of a purchase; it derives eligible tickers from those saved records. Each purchase is compared with equal-dollar installments centered on its date, and recent purchases wait for a complete window. Yahoo receives only public tickers, a date range extended from the preset period boundary, and daily-price options, never purchase amounts or individual transaction dates. A separate bounded cache retains public adjusted prices and instrument metadata for up to 15 minutes. Private purchase budgets, per-purchase schedules, and comparison results are calculated on demand, returned with `no-store`, and are not retained in that cache or written back to account history. The model identifies known exclusions and posting-date approximations and does not initiate trades.
...
Losing the AES key makes existing encrypted account, bonus, card, Plaid, and E*TRADE records unrecoverable. Replacing `CARDDUE_MASTER_KEY` does not rotate existing ciphertext; this release has no automatic bulk re-encryption workflow. Provider free-tier retention and availability are not permanent guarantees.
...
- Plaid, E*TRADE, Yahoo Finance, and connected institutions process information under their own terms and privacy policies.
- Account balances, holdings, liability fields, transactions, open orders, market prices, and reconstructed history can be delayed, missing, modified, or incorrect. Estimated history is not broker-reported performance. ChipDue never initiates or guarantees a payment or trade.
- Vercel, Neon, Google, Plaid, E*TRADE, and their free-tier terms or identity policies can change. Local manual mode remains provider-independent.

## Delete your data

If Plaid is connected, disconnect each Item inside ChipDue first so the application can call Plaid's `/item/remove` endpoint. Disconnect E*TRADE so ChipDue can attempt token revocation, then choose **Forget credentials**. Then remove the associated ChipDue records.

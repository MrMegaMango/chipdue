...
It does not store full account numbers, credentials entered into Plaid Link, identity profiles, addresses, locations, original bank descriptions, merchant logos, counterparties, security-level investment positions, raw Plaid responses, or E*TRADE order results. Plaid responses are mapped immediately to the allowlisted account, card, and transaction fields, encrypted as application records, and then discarded. E*TRADE account and order responses are reduced to allowlisted display fields and kept only in browser memory for the current page.
...
- Plaid, E*TRADE, and connected institutions process information under their own terms and privacy policies.
- Account balances, holdings, liability fields, transactions, and open orders can be delayed, missing, modified, or incorrect. ChipDue never initiates or guarantees a payment or trade.
